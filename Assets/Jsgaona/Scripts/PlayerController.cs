...
namespace Jsgaona {
    ...
    public class PlayerController : MonoBehaviour {
        ...
        public Joystick JoystickController { set { joystick = value; } get { return joystick; } }
        ...
        private void Update() {
            if(enableControl) {
                ...
                if (!isDashing) {
                    dirMove = MoveRelative().normalized;
        ...
        private Vector3 MoveTopDawn() {
            return new Vector3(joystick.Horizontal, 0, joystick.Vertical);
        }


        private Vector3 MoveRelative() {
            // Vector3 move = (transform.forward * joystick.Horizontal) + (transform.right * joystick.Vertical);
            ...
            Vector3 desiredMove = camForward * input.z + camRight * input.x;
            if (desiredMove.sqrMagnitude > 1e-4f) desiredMove.Normalize();

            if (inputMag > 0.01f) {
                if (!isAiming) {
                    // Free-walk: gira hacia la dirección de MOVIMIENTO (GTA walk)
                    ...
                    // Aim/lock: el PJ mira donde mira la cámara (solo yaw)
