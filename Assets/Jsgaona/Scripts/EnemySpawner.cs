...
namespace Assets.JSGAONA.Unidad1.Scripts
{
    ...
    //Variables visibles desde el inspector de Unity
    {
        ...
        [SerializeField] private float[] distance = { 10f, 20f, 40f, float.MaxValue };
        ...
        //Metodo de llamada de Unity, se lalma una unica vez al iniciar el app, se ejecuta despues
        ...
        private void Start()
        {
            ...
            //Se recorre todos la lista para inicializar con nuevos objetos BoundingSphere
            for(int i = 0; i < boundingSpheres.Length; i++)
            {
                boundingSpheres[i] = new BoundingSphere(points[i].position, radio);
            }

            //Se gestiona de manera eficiente la activacion y desactivacion de objetos en funcion
        ...
        private void SpawnPointCullingGroupChanged (CullingGroupEvent sphere)
        {
            ...
            if(enemyExist && sphere.currentDistance <= 1)
            {
            ...
            //Desactiva el enemigo si el personaje se aleja demasiado
